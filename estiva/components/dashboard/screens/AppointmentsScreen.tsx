  ...
  return (
    <div className="space-y-5 text-white">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard label={t.scheduled} value={stats.scheduled} color="bg-blue-400" />
        <StatCard label={t.confirmed} value={stats.confirmed} color="bg-emerald-400" />
        <StatCard label={t.completed} value={stats.completed} color="bg-green-400" />
        <StatCard label={t.cancelled} value={stats.cancelled} color="bg-red-400" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {Object.entries(STATUS_MAP).map(([key, val]) => (
            <option key={key} value={key} className="bg-[#1a1a2e]">{language === "tr" ? val.tr : val.en}</option>
          ))}
        </select>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-20 shrink-0 text-center">
                      <p className="text-sm font-bold text-white">{formatTime(apt.startTime)}</p>
                      <p className="text-[10px] text-white/30">{formatTime(apt.endTime)}</p>
                    </div>
                    ...
                    <div className="flex shrink-0 gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-lg text-white/40 transition hover:bg-[#25D366]/20 hover:text-[#25D366]"
                      ...
                      <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-lg text-white/40 transition hover:bg-red-500/20 hover:text-red-400"
      ...
      {viewMode === "timeline" && (
        <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
          {loading ? (
            ...
            <div className="p-12 text-center text-white/40">{t.noStaff}</div>
          ) : (
            <div className="overflow-x-auto">
              <div className="min-w-[800px]">
                {/* Staff header */}
                <div className="sticky top-0 z-10 flex border-b border-white/[0.06] bg-[#0d0d1a]/90 backdrop-blur-sm">
                  <div className="w-16 shrink-0 border-r border-white/[0.06] px-2 py-3 text-[10px] font-medium text-white/30" />
                  {staffWithAppointments.map((s) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-white">{s.name} {s.surname}</p>
                          <p className="text-[10px] text-white/30">{s.appointments.length} {language === "tr" ? "randevu" : "appts"}</p>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {showCustomerDropdown && (
                  <div className="absolute left-0 right-0 z-20 mt-1 max-h-48 overflow-y-auto rounded-xl border border-white/10 bg-[#1a1a2e] shadow-xl">
                    {filteredCustomers.length === 0 ? (
                      <div className="px-4 py-3 text-xs text-white/30">{t.noData}</div>
                    ) : (
                      filteredCustomers.map((c) => (
                        <button
                          ...
                          className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition hover:bg-white/5 ${form.customerId === c.id ? "bg-white/5" : ""}`}
                        >
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-medium text-white">{t.recurring}</span>
            </label>

            {form.isRecurring && (
              <div className="mt-4 flex items-center gap-3">
                <span className="text-xs text-white/50">{t.interval}</span>
                <input
                  type="number"
                  min={1}
                  max={365}
                  ...
                  className="w-20 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1.5 text-center text-sm text-white focus:outline-none"
                />
                <span className="text-xs text-white/50">{t.days}</span>
                <div className="mx-2 h-4 w-px bg-white/10" />
                <span className="text-xs text-white/50">{t.sessions}</span>
                <input
                  type="number"
                  min={2}
                  max={52}
          ...
          return (
            <div className="space-y-5">
              ...
              <div className={`flex items-center gap-3 rounded-xl border p-3 ${status.bg}`}>
                ...
                {apt.isRecurring && (
                  <span className="ml-auto rounded-md bg-white/10 px-2 py-0.5 text-[10px] font-bold">
                    {t.session} {apt.sessionNumber}/{apt.totalSessions}
                  </span>
                )}
              </div>
                    ...
                    {apt.seriesAppointments.map((sa) => {
                      ...
                      return (
                        <div key={sa.id} className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-xs ${sa.id === apt.id ? "bg-white/10" : ""}`}>
                          ...
                          <span className="text-white/50">{t.session} {sa.sessionNumber}</span>
                          <span className="text-white">{formatDateShort(sa.startTime)}</span>
                          <span className="text-white/40">{formatTime(sa.startTime)}</span>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl bg-gradient-to-r from-[#00a651] to-[#00c853] px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-green-900/30 transition hover:shadow-green-900/50"
            >
              {t.updateStatus}
            </button>
            <button
              onClick={() => setShowStatusModal(false)}
              className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-medium text-white/60 transition hover:bg-white/5"
            >
              {t.cancel}
            </button>
          </div>

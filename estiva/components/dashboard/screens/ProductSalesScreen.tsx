    ...
    <div className={`space-y-5 ${isDark ? "text-white" : "text-gray-900"}`}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {tab === "sales" && (<>
          ...
          <LocaleDateInput value={endDate} onChange={e => setEndDate(e.target.value)} className={`rounded-xl border ${isDark ? "border-white/[0.08]" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} px-3 py-1.5 text-xs ${isDark ? "text-white" : "text-gray-900"} focus:outline-none`} isDark={isDark} />
          <select value={staffFilter} onChange={e => setStaffFilter(e.target.value === "" ? "" : Number(e.target.value))} className={`rounded-xl border ${isDark ? "border-white/[0.08]" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} px-3 py-1.5 text-xs ${isDark ? "text-white" : "text-gray-900"} focus:outline-none`}><option value="" className={`${isDark ? "bg-[#1a1a2e]" : "bg-white"}`}>{t.allStaff}</option>{staffList.filter(s => s.isActive).map(s => <option key={s.id} value={s.id} className={`${isDark ? "bg-[#1a1a2e]" : "bg-white"}`}>{s.name} {s.surname}</option>)}</select>
        </>)}
        <div className="relative ml-auto"><svg className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? "text-white/30" : "text-gray-300"}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg><input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t.search} className={`rounded-xl border ${isDark ? "border-white/[0.08]" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} py-1.5 pl-11 pr-3 text-xs ${isDark ? "text-white" : "text-gray-900"} ${isDark ? "placeholder:text-white/30" : "placeholder:text-gray-400"} focus:outline-none w-48`} /></div>
      </div>
      ...
      {tab === "sales" && (
        <div className={`overflow-hidden rounded-2xl border ${isDark ? "border-white/[0.06]" : "border-gray-200"} ${isDark ? "bg-white/[0.02]" : "bg-white"} ${isDark ? "shadow-[0_8px_32px_rgba(0,0,0,0.3)]" : "shadow-sm"}`}>
          {loading ? (<div className={`flex items-center justify-center gap-3 p-12 ${isDark ? "text-white/40" : "text-gray-400"}`}><div className={`h-5 w-5 animate-spin rounded-full border-2 ${isDark ? "border-white/20 border-t-white/60" : "border-gray-200 border-t-gray-500"}`} />{t.loading}</div>
            ...
            <div className={`flex items-center justify-end border-t ${isDark ? "border-white/[0.06]" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} px-5 py-3`}><span className="text-sm font-bold text-emerald-400">₺{fmt(totalRevenue)}</span></div>
          </>)}
        </div>
      )}
      ...
      <Modal open={showSaleForm} onClose={() => setShowSaleForm(false)} title={t.createSaleTitle} maxWidth="max-w-xl">
        <form onSubmit={handleSaleSubmit} className="space-y-5">
            ...
            <div className={`space-y-1.5 max-h-40 overflow-y-auto rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} p-2`}>
              {products.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-4 text-center">
                  ...
                  <p className={`mt-1 text-[10px] ${isDark ? "text-white/25" : "text-gray-400"}`}>{language === "tr" ? "Önce \"Ürünler\" sayfasından ürün ekleyin" : "Add products from the \"Products\" page first"}</p>
                </div>
              ) : (
                products.map(p => (<button key={p.id} type="button" onClick={() => setSaleForm({ ...saleForm, productId: p.id })} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left transition ${saleForm.productId === p.id ? isDark ? "bg-white/10 ring-1 ring-white/20" : "bg-gray-100 ring-1 ring-gray-300" : isDark ? "hover:bg-white/5" : "hover:bg-gray-50"}`}><div><p className={`text-xs font-medium ${isDark ? "text-white" : "text-gray-900"}`}>{p.name}</p><p className={`text-[10px] ${isDark ? "text-white/30" : "text-gray-300"}`}>₺{fmt(p.price)} • {p.stockQuantity} {t.pcs}</p></div>{saleForm.productId === p.id && <svg className="shrink-0 text-emerald-400" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}</button>))
              )}
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="relative" ref={customerDdRef}>
                <input type="text" value={customerSearch} onChange={e => { setCustomerSearch(e.target.value); setShowCustomerDd(true); }} onFocus={() => setShowCustomerDd(true)} placeholder={t.searchCustomer} className={`w-full rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/5" : "bg-gray-50"} px-3 py-2.5 text-sm ${isDark ? "text-white" : "text-gray-900"} ${isDark ? "placeholder:text-white/30" : "placeholder:text-gray-400"} focus:outline-none ${isDark ? "focus:border-white/25" : "focus:border-gray-400"}`} />
            ...
            <div className="space-y-2">
              ...
              <input type="number" min={1} value={saleForm.quantity} onChange={e => setSaleForm({ ...saleForm, quantity: Number(e.target.value) })} className={`w-full rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/5" : "bg-gray-50"} px-3 py-2.5 text-sm ${isDark ? "text-white" : "text-gray-900"} focus:outline-none ${isDark ? "focus:border-white/25" : "focus:border-gray-400"}`} />
          ...
          <div className="space-y-2"><label className={`text-xs font-semibold tracking-wider ${isDark ? "text-white/40" : "text-gray-400"}`}>{t.notes}</label><input type="text" value={saleForm.notes} onChange={e => setSaleForm({ ...saleForm, notes: e.target.value })} placeholder={t.notesPlaceholder} className={`w-full rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/5" : "bg-gray-50"} px-3 py-2.5 text-sm ${isDark ? "text-white" : "text-gray-900"} ${isDark ? "placeholder:text-white/30" : "placeholder:text-gray-400"} focus:outline-none ${isDark ? "focus:border-white/25" : "focus:border-gray-400"}`} /></div>
      ...
      <Modal open={showProductForm} onClose={() => setShowProductForm(false)} title={productFormMode === "create" ? t.createProductTitle : t.editProductTitle} maxWidth="max-w-md">
        <form onSubmit={handleProductSubmit} className="space-y-5">
          <div className="space-y-2"><label className={`text-xs font-semibold tracking-wider ${isDark ? "text-white/40" : "text-gray-400"}`}>{t.productName}</label><input type="text" value={productForm.name} onChange={e => setProductForm({ ...productForm, name: e.target.value })} placeholder={t.productNamePh} className={`w-full rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/5" : "bg-gray-50"} px-3 py-2.5 text-sm ${isDark ? "text-white" : "text-gray-900"} ${isDark ? "placeholder:text-white/30" : "placeholder:text-gray-400"} focus:outline-none ${isDark ? "focus:border-white/25" : "focus:border-gray-400"}`} /></div>
          <div className="space-y-2"><label className={`text-xs font-semibold tracking-wider ${isDark ? "text-white/40" : "text-gray-400"}`}>{t.description}</label><input type="text" value={productForm.description} onChange={e => setProductForm({ ...productForm, description: e.target.value })} placeholder={t.descPh} className={`w-full rounded-xl border ${isDark ? "border-white/10" : "border-gray-200"} ${isDark ? "bg-white/5" : "bg-gray-50"} px-3 py-2.5 text-sm ${isDark ? "text-white" : "text-gray-900"} ${isDark ? "placeholder:text-white/30" : "placeholder:text-gray-400"} focus:outline-none ${isDark ? "focus:border-white/25" : "focus:border-gray-400"}`} /></div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      ...
      <Modal open={showSaleDetail} onClose={() => setShowSaleDetail(false)} title={t.saleDetailTitle} maxWidth="max-w-md">
        {selectedSale && (() => { const s = selectedSale; return (
          <div className="space-y-5">
            <div className={`flex flex-col items-center gap-1 rounded-xl border ${isDark ? "border-white/[0.06]" : "border-gray-200"} ${isDark ? "bg-white/[0.03]" : "bg-gray-50/50"} py-5`}><p className={`text-3xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>{s.currencySymbol}{fmt(s.totalAmount)}</p>{s.currencyCode !== "TRY" && <p className={`text-sm ${isDark ? "text-white/40" : "text-gray-400"}`}>₺{fmt(s.amountInTry)}</p>}<MethodBadge display={s.paymentMethodDisplay} language={language} /></div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1"><p className={`text-[10px] font-semibold tracking-wider ${isDark ? "text-white/30" : "text-gray-300"}`}>{t.saleDate}</p><p className={`text-sm ${isDark ? "text-white" : "text-gray-900"}`}>{formatDate(s.saleDate, language)} {formatTime(s.saleDate, language)}</p></div>

...
export default function StatCard({
  ...
  return (
    ...
    >
      {/* Ambient glow */}
      <div
        className={`absolute -right-4 -top-4 h-24 w-24 rounded-full opacity-20 blur-2xl transition-all duration-500 group-hover:h-32 group-hover:w-32 group-hover:opacity-40 ${gradient}`}
      />
      {/* Shimmer sweep */}
      ...
      <div className="relative flex items-start gap-3">
        ...
        {icon && (
          <div
            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110 ${gradient} bg-opacity-20 ${iconColor}`}
          >
            {icon}
          </div>
        )}

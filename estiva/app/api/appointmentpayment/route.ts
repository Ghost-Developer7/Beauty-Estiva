...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const items = payments.map((p) => ({
      ...
      customerFullName: `${p.Appointments.Customers.Name} ${p.Appointments.Customers.Surname}`,
      treatmentName: p.Appointments.Treatments.Name,
      staffFullName: `${p.Appointments.Users.Name} ${p.Appointments.Users.Surname}`,
      appointmentStartTime: p.Appointments.StartTime,
      amount: Number(p.Amount),
      currencyCode: p.Currencies.Code,
      currencySymbol: p.Currencies.Symbol,
      exchangeRateToTry: Number(p.ExchangeRateToTry),
      amountInTry: Number(p.AmountInTry),

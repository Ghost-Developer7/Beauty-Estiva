...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!recipients || !Array.isArray(recipients) || recipients.length === 0) {
      return fail("recipients dizisi gereklidir");
    }
    ...
    try {
      ...
      return success({ response: result, sentTo: recipients.length }, "SMS gönderildi");
    } catch (smsError) {
      console.error("SMS bulk send error:", smsError);
      return fail("SMS gönderilirken hata oluştu");
    }
  } catch (error) {
    console.error("SMS send-bulk POST error:", error);
    return serverError();
  }
}
